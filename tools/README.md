...
## Отличие от indicator

`indicator` использует `uart_process_rx()` — тихо отбрасывает все ошибки,
вызывает коллбэк только для валидных фреймов. Это правильное поведение для
продакшн-демона.

`uart_rx_dump` реализует собственный цикл разбора поверх `protocol_parse_frame()`
напрямую, без фильтрации. Любое отклонение от ожидаемого видно сразу.
