...
static size_t make_frame(const char *payload, uint8_t *buf) {
    ...
    buf[i++] = opcode;
    memcpy(buf + i, payload, data_len); i += data_len;
...
void test_parse_frame_valid(void) {
    ...
    uint8_t buf[512]; size_t flen = make_frame(payload, buf);
    mu_frame_t frame; size_t consumed = 0;
...
void test_parse_frame_garbage_before_sync1(void) {
    ...
    raw[0]=0x00; raw[1]=0x99; memcpy(raw+2, fbuf, flen);
    mu_frame_t frame; size_t consumed = 0;
...
void test_parse_frame_two_frames(void) {
    ...
    TEST_ASSERT_EQUAL(PARSE_OK, protocol_parse_frame(buf+consumed, f1+f2-consumed, &frame, &consumed));
...
void test_parse_payload_basic(void) {
    const char *payload = "#STM:L10:R3:A1:S4:M0:E#\r\n";
    uint8_t raw[512]; size_t flen = make_frame(payload, raw);
    mu_frame_t frame; size_t consumed = 0;
