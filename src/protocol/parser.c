...
parse_result_t protocol_parse_frame(const uint8_t *buf, size_t len,
    ...
    *consumed = 0u;
    ...
    size_t start = 0u;
    while (start < len && buf[start] != MU_SYNC1) { start++; }
    ...
    crc_input[0u] = opcode;
    if (data_size > 0u) { memcpy(crc_input + 1u, data_ptr, data_size); }

    uint16_t crc_calc = protocol_crc16(crc_input, 1u + (size_t)data_size);
    uint16_t crc_recv =
        ((uint16_t)buf[start + 3u + data_size] << 8u) |
         (uint16_t)buf[start + 4u + data_size];
    ...
    if (data_size > 0u) { memcpy(out->data, data_ptr, data_size); }
...
parse_result_t protocol_parse_payload(const mu_frame_t *frame,
    ...
    for (int i = 0; i < 5; i++) {
        if (*p != PREFIXES[i]) { return PARSE_ERROR_PAYLOAD; }
        p++;
        char *end;
        long val = strtol(p, &end, 10);
        if (end == p)           { return PARSE_ERROR_PAYLOAD; }
        values[i] = (int)val;
        p = end;
        if (i < 4) {
            if (*p != ':') { return PARSE_ERROR_PAYLOAD; }
            p++;
        }
    }
    ...
    if (values[0] < 0 || values[0] > (int)CHAR_CODE_MAX)  return PARSE_ERROR_RANGE;
    if (values[1] < 0 || values[1] > (int)CHAR_CODE_MAX)  return PARSE_ERROR_RANGE;
    if (values[2] < 0 || values[2] > (int)ARROW_CODE_MAX) return PARSE_ERROR_RANGE;
    if (values[3] < 0 || values[3] > (int)SOUND_CODE_MAX) return PARSE_ERROR_RANGE;
    /* mode_t: нельзя проверять <= MAX из-за разрыва 0–9, 100, 101, 255 */
    if (!mode_is_valid(values[4]))                         return PARSE_ERROR_RANGE;
