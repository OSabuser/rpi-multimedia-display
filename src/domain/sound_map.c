...
static void seq_add(audio_sequence_t *seq, const char *filename) {
    if (seq->count >= AUDIO_SEQ_MAX_FILES) return;
...
static void resolve_floor_announcement(floor_t floor, audio_sequence_t *out) {
    ...
    case FLOOR_TYPE_NORMAL: {
        int n = floor.number;
        if      (n >= 1  && n <= 20) { seq_add_num(out, n);                         seq_add(out, "floor.wav"); }
        else if (n >= 21 && n <= 29) { seq_add(out, "20-.wav"); seq_add_num(out, n % 10); seq_add(out, "floor.wav"); }
        else if (n == 30)            { seq_add(out, "30.wav");                       seq_add(out, "floor.wav"); }
        else if (n >= 31 && n <= 39) { seq_add(out, "30-.wav"); seq_add_num(out, n % 10); seq_add(out, "floor.wav"); }
        else if (n == 40)            { seq_add(out, "40.wav");                       seq_add(out, "floor.wav"); }
        else                         { seq_add(out, "g_triple.wav"); }
        break;
    }
